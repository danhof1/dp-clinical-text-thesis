...
def run_relearn_attack(checkpoint_path: str, tokenizer, splits_path: str,
    ...
    # Load a fresh copy of the checkpoint — never modify the original
    model = AutoModelForCausalLM.from_pretrained(
        checkpoint_path, torch_dtype=torch.bfloat16, attn_implementation="sdpa",
    ).to(device)
    model.eval()
...
class Evaluator:
    ...
    def run_suite(self, checkpoint_path: str, model=None) -> dict:
        ...
        if model is None:
            log.info("Loading model from %s for eval suite", checkpoint_path)
            _model, _ = load_model_and_tokenizer(checkpoint_path,
                                                  bf16=self.cfg.get("bf16", True))
            _model = _model.to(self.device).eval()
            _owns_model = True
        else:
            _model = model
            _owns_model = False
