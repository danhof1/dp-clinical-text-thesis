...
class RMUConfig:
    ...
    layer_ids: tuple = ()       # multi-layer override; if non-empty, layer_id is ignored
    alpha: float = 1200.0       # weight on forget misdirection loss
    steering_coeff: float = 20.0  # L2 norm of the random target vector (or multiplier in adaptive mode)
    adaptive_steering: bool = False  # scale steering_coeff per layer to match hidden state norms
    ...
    seed: int = 42
    log_every: int = 10
    save_merged: bool = True  # False = save adapter only (~160MB vs 14GB)
    eval_config_path: str = ""   # path to evaluator YAML; empty = skip
...
@torch.no_grad()
def compute_layer_norms(model, loader, layer_ids, device, n_batches=5):
    """Mean per-token L2 norm of hidden states at each target layer (Dang et al. AAAI 2025)."""
    model.eval()
    norms = {lid: [] for lid in layer_ids}
    for i, batch in enumerate(loader):
        ...
        for lid in layer_ids:
            hidden = outputs.hidden_states[lid + 1]
            token_norms = hidden[mask].norm(dim=-1)
            norms[lid].append(token_norms.mean().item())
    model.train()
    return {lid: sum(ns) / len(ns) for lid, ns in norms.items()}


def forget_loss(model, batch: dict, layer_ids: list, steering_coeffs: dict) -> torch.Tensor:
    """
    MSE between hidden states and independent random vectors at each target layer.
    Single forward pass; returns mean loss across all target layers.
    """
    outputs = model(
        input_ids=batch["input_ids"],
        attention_mask=batch["attention_mask"],
...
def run(cfg: RMUConfig) -> None:
    ...
    # -- Evaluator (optional) --
    _evaluator = None
    if cfg.eval_config_path:
        try:
            import yaml as _yaml
            from eval.evaluator import Evaluator as _Eval
            with open(cfg.eval_config_path) as _f:
                _ecfg = _yaml.safe_load(_f)
            _evaluator = _Eval(_ecfg, tokenizer, device=str(device))
            log.info("Evaluator loaded from %s", cfg.eval_config_path)
        except Exception as _e:
            log.warning("Evaluator init failed (training will continue): %s", _e)
    # -- end evaluator init --
    ...
    _lids = list(cfg.layer_ids) if cfg.layer_ids else [cfg.layer_id]

    if cfg.adaptive_steering:
        layer_norms = compute_layer_norms(model, forget_loader, _lids, device)
        steering_coeffs = {lid: cfg.steering_coeff * norm for lid, norm in layer_norms.items()}
        log.info("Adaptive steering -- layer norms: %s",
                 {lid: f"{n:.1f}" for lid, n in layer_norms.items()})
        log.info("Adaptive steering -- coefficients: %s",
                 {lid: f"{c:.1f}" for lid, c in steering_coeffs.items()})
    else:
        steering_coeffs = {lid: cfg.steering_coeff for lid in _lids}

    log.info(
        "RMU: %d steps | layer_ids=%s | alpha=%.0f | adaptive=%s",
        cfg.num_steps, _lids, cfg.alpha, cfg.adaptive_steering,
    )
    ...
    for step in range(1, cfg.num_steps + 1):
        ...
        layer_ids = list(cfg.layer_ids) if cfg.layer_ids else [cfg.layer_id]
        f_loss = forget_loss(model, f_batch, layer_ids, steering_coeffs)
        ...
        # -- mid-training PPL probe --
        if _evaluator is not None and _evaluator.metrics.get("ppl_on_splits"):
            _evaluator.run_ppl_probe(model, step)
        # -- end probe --
    ...
    if _evaluator is not None:
        log.info("Running post-training evaluation suite...")
        try:
            _evaluator.run_suite(str(ckpt_dir), model=None)
        except Exception as _e:
            log.error("Eval suite failed (checkpoint safe): %s", _e)
    # -- end eval suite --
