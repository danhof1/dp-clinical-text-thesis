#!/usr/bin/env python3
"""
Section-wise generation for Term2Note-trained models (v3).

Changes from v2:
  - Saves all k candidates per section (--save_candidates)
  - Term-recall scoring: combined PPL + conditioning term overlap
  - alpha=1.0 (default) reproduces v2 behavior exactly
...
def term_recall_score(text, terms_str):
    """Fraction of conditioning SNOMED terms present in generated text."""
    if not terms_str or terms_str == "none":
        return 0.0
...
def combined_section_score(ppl, term_recall, alpha, ppl_min, ppl_max):
    """Higher = better. alpha=1 is pure PPL (inverted), alpha=0 is pure term recall."""
    if ppl_max == ppl_min:
        norm_ppl = 0.5
    else:
        norm_ppl = 1.0 - (ppl - ppl_min) / (ppl_max - ppl_min)
    norm_ppl = max(0.0, min(1.0, norm_ppl))
    return alpha * norm_ppl + (1.0 - alpha) * term_recall
...
def generate_note(control_codes, icd_term_sets,
                  ...
                  rng, k=4, save_candidates=False, alpha=1.0):
    ...
    for group_name in SECTION_GROUPS:
        ...
        for cand in candidates:
            ...
            tr = term_recall_score(cand, terms_str)
            scored.append((cand, ppl, tr))

        all_cands_save = []
        if save_candidates:
            for cand, ppl, tr in scored:
                all_cands_save.append({"text": cand, "ppl": ppl, "term_recall": tr})
        ...
        if not scored:
            ...
            best_text, best_ppl, best_tr = scored[0]
        else:
            ppls = [s[1] for s in scored if s[1] < float("inf")]
            ppl_min = min(ppls) if ppls else 0
            ppl_max = max(ppls) if ppls else 1
            scored_cs = [
                (c, p, tr, combined_section_score(p, tr, alpha, ppl_min, ppl_max))
                for c, p, tr in scored
            ]
            scored_cs.sort(key=lambda x: -x[3])
            best_text, best_ppl, best_tr, _ = scored_cs[0]
        ...
        if save_candidates:
            sec_rec["candidates"] = all_cands_save
        sections_generated.append(sec_rec)
...
def run(args):
    ...
    stats = {"total": 0, "section_ppls": defaultdict(list), "section_term_recalls": defaultdict(list)}
    ...
    with open(out_path, "a") as f_out:
        for i in range(args.n_generate):
            ...
            full_text, sections = generate_note(
                cc, icd_term_sets,
                model, ppl_model, tokenizer, ppl_tokenizer, device,
                rng, k=args.k, save_candidates=args.save_candidates,
                alpha=args.alpha,
            )
            ...
            for s in sections:
                ...
                stats["section_term_recalls"][s["group"]].append(s.get("term_recall", 0.0))
    ...
    summary = {
        ...
        "section_mean_term_recall": {
            g: sum(v) / len(v) if v else 0
            for g, v in stats["section_term_recalls"].items()
        },
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_candidates", action="store_true",
                        help="Save all k candidates per section for offline reranking")
    parser.add_argument("--alpha", type=float, default=1.0,
                        help="PPL vs term-recall weight (1.0=pure PPL, 0=pure term recall)")
